...
# 1. Add CSS for new elements before </style>
css_additions = """
    ...
    .temple-bell {
      position: absolute; top: 0; width: 45px; height: 60px;
    ...
    .banana-leaf.left { bottom: -20px; left: -20px; transform: rotate(15deg); }
    .banana-leaf.right { bottom: -20px; right: -20px; transform: scaleX(-1) rotate(15deg); }
...
# 2. Add texture overlay to card panels
...
# 3. Inject Thoranam and Temple Bells into hero
hero_additions = """
  <section id="hero">
...
"""
content = content.replace('<section id="hero">', hero_additions)

# 4. Replace generic divider with Kolam
kolam_svg = """
...
"""
content = re.sub(r'<div class="drawing-divider" id="divider-event">.*?</div>', kolam_svg, content, flags=re.DOTALL)
...
"""
content = re.sub(r'<div class="drawing-divider" id="divider-countdown">.*?</div>', kolam_svg_2, content, flags=re.DOTALL)

# 5. Add Kuthuvilakku to Event Details
kuthu_svg = """
...
"""
content = content.replace('<div class="event-grid">', kuthu_svg + '\n      <div class="event-grid">')

# 6. Add Banana leaves to Event Section
banana_leaves = """
  <!-- EVENT DETAILS -->
...
"""
content = content.replace('<!-- EVENT DETAILS -->\n  <section id="event-details">', banana_leaves)

# 7. Update JS for Mouse Tilt on Card
js_tilt = """
  /* ---- MOUSE TILT FOR CARD ---- */
...
"""
content = content.replace('/* ---- MOUSE PARALLAX EFFECT ---- */', js_tilt)

# 8. Reset card transform on open
js_open = """
  /* ---- 3D BOOKLET ACTIONS ---- */
...
"""
content = content.replace("openBtn.addEventListener('click', function() {", js_open)

# 9. Update JS falling petals to include Jasmine
